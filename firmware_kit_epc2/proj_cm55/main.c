...
#include "app/drone_hunter/drone_hunter_arena.h"
...
#ifndef APP_DRONE_HUNTER_MODE
#define APP_DRONE_HUNTER_MODE               (1U)
#endif
...
static void cm55_gfx_task(void *arg)
{
    ...
    if (CY_GFX_SUCCESS == gfx_status)
    {
        ...
        if (VG_LITE_SUCCESS == vglite_status)
        {
            ...
            lv_port_disp_init();
            lv_port_indev_init();
            #if (APP_DRONE_HUNTER_MODE == 1U)
            drone_hunter_arena_start(lv_screen_active());
            #else
            lv_demo_music();
            #endif
        }
